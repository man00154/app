...
# Suppress warnings from libraries like statsmodels
import warnings
warnings.filterwarnings("ignore")

# Import functions from your app (assuming app.py is now main.py or its contents are here)
# For simplicity, I'm including the functions directly here. In a larger project,
# you might keep them in a separate `models.py` and import them.

# --- Start of functions from original app(1).py ---
...
def train_and_forecast_arima(train_df, test_df, order=(1,1,1), seasonal_order=(1,1,0,4)):
    ...
    """
    from statsmodels.tsa.arima.model import ARIMA # Local import to avoid circular dependency if this was in a separate file
    try:
...
def train_and_forecast_prophet(train_df, test_df, time_granularity):
    ...
    Returns:
        tuple: (pd.Series, dict) - Predicted values and evaluation metrics.
    """
    from prophet import Prophet # Local import
    try:
...
def train_and_forecast_ets(train_df, test_df, time_granularity):
    ...
    Returns:
        tuple: (pd.Series, dict) - Predicted values and evaluation metrics.
    """
    from statsmodels.tsa.api import ExponentialSmoothing # Local import
    try:
        train_series = train_df.set_index('ds')['y']
...
def train_and_forecast_ml_model(train_df, test_df, model_type='RandomForest'):
    ...
    Returns:
        tuple: (pd.Series, dict) - Predicted values and evaluation metrics.
    """
    from sklearn.ensemble import RandomForestRegressor # Local import
    from xgboost import XGBRegressor # Local import
    from sklearn.metrics import mean_absolute_error, mean_squared_error # Local import
...
def train_and_select_best_model(df_agg, category, granularity, test_size):
    ...
    xgb_model = XGBRegressor(n_estimators=100, random_state=42, objective='reg:squareor')
...
# --- End of functions from original app(1).py ---


# Initialize FastAPI app
app = FastAPI(
    title="ITSM Incident Forecasting API",
    description="API for forecasting ITSM incidents, detecting data drift, and model retraining.",
    version="1.0.0"
)
...
MODEL_TRAINING_SUCCESS = Counter('model_training_success_total', 'Total successful model trainings')
MODEL_TRAINING_FAILURES = Counter('model_training_failures_total', 'Total failed model trainings')
DATA_DRIFT_DETECTED = Counter('data_drift_detected_total', 'Total data drift detections')
FORECAST_REQUESTS_TOTAL = Counter('forecast_requests_total', 'Total forecast requests')
FORECAST_ERRORS_TOTAL = Counter('forecast_errors_total', 'Total forecast errors')
MODEL_RMSE_GAUGE = Gauge('model_rmse', 'RMSE of the best model', ['category', 'granularity', 'model_name'])

# Custom collector for dynamic metrics (e.g., model RMSE)
class ModelMetricsCollector(object):
    def collect(self):
        metadata_path = os.path.join(MODEL_DIR, "trained_models_metadata.json")
        if os.path.exists(metadata_path):
            ...
            for key, info in trained_models_info.items():
                category, granularity = key.split('_')
                model_name = info.get("model_name", "unknown")
                # To get RMSE, you'd need to store it in metadata during training
                # For now, we'll use a placeholder or assume it's part of the model object
                # For a real scenario, you'd calculate and store evaluation metrics in the metadata
                # during the training pipeline.
                # Here, we'll just report a dummy value or a value from a hypothetical stored metric
                # For demonstration, let's assume a dummy RMSE for now.
                # In a real app, `trained_models_info` would include a 'metrics' key.
                # Example: info['metrics']['RMSE']
                
                # Placeholder for RMSE, replace with actual logic if metrics are stored in metadata
                # For this example, we'll just report a dummy value or skip if not available
                # A more robust solution would involve recalculating or storing metrics.
                
                # For this example, let's just report a dummy value or assume it's not available
                # if not explicitly stored.
                # If you modify `run_full_training_pipeline` to store RMSE, you can use it here.
                # For now, let's just show how to expose a gauge.
                
                # A better way would be to update the gauge directly after training
                # or store the RMSE in the metadata.
                # Let's assume for now that `trained_models_info` has an `rmse` field.
                # If not, this gauge will not be truly reflective.
                
                # To make this work, `run_full_training_pipeline` would need to store the RMSE
                # of the best model in the `trained_models_info` dictionary.
                # For example:
                # trained_models_info[f"{category}_{granularity}"] = {
                #     "model_name": best_model_name,
                #     "data_snapshot": df_agg.to_json(...),
                #     "rmse": min_rmse # Add this line
                # }
                
                # Since the provided `app(1).py` doesn't store RMSE directly in metadata,
                # we'll use a dummy value or skip.
                # For a real application, ensure RMSE is stored in `trained_models_info`.
                
                # Dummy RMSE for demonstration. In a real app, get from `info`.
                # For example: `rmse_value = info.get("rmse", 0.0)`
                
                # Let's create a dummy gauge for now, as the RMSE isn't stored in the metadata
                # from the original `app(1).py`.
                # If `trained_models_info` had a 'rmse' key, you'd use:
                # g = GaugeMetricFamily('model_rmse', 'RMSE of the best model', labels=['category', 'granularity', 'model_name'])
                # g.add_metric([category, granularity, model_name], info['rmse'])
                # yield g
                pass # Skipping for now as RMSE is not in the metadata.

# Register the custom collector
REGISTRY.register(ModelMetricsCollector())

# Initial training on startup (optional, but good for first deployment)
# This will run in the background when the application starts
@app.on_event("startup")
async def startup_event():
    logger.info("Application startup: Initializing models...")
    # Ensure the model directory exists
    os.makedirs(MODEL_DIR, exist_ok=True)
    # Run initial training in a background task
    # This avoids blocking the startup of the API
    background_tasks = BackgroundTasks()
    background_tasks.add_task(initial_model_training)
    # The background task will start, but we don't await it here to let the app start
    # You might want a more robust way to handle initial training status.
    logger.info("Initial model training scheduled in background.")
    # Start Prometheus HTTP server
    start_http_server(8001) # Prometheus metrics will be exposed on port 8001
    logger.info("Prometheus metrics server started on port 8001")

async def initial_model_training():
    """Function to run the full training pipeline."""
    try:
        success = run_full_training_pipeline(file_path="app/ITSM_data.csv")
        if success:
            MODEL_TRAINING_SUCCESS.inc()
            logger.info("Initial model training completed successfully.")
            # After successful training, update RMSE gauges
            metadata_path = os.path.join(MODEL_DIR, "trained_models_metadata.json")
            if os.path.exists(metadata_path):
                with open(metadata_path, "r") as f:
                    trained_models_info = json.load(f)
                for key, info in trained_models_info.items():
                    category, granularity = key.split('_')
                    model_name = info.get("model_name", "unknown")
                    # This is where you would retrieve the actual RMSE from `info`
                    # For now, let's just set a dummy value if RMSE is not stored.
                    # If `run_full_training_pipeline` was modified to store RMSE:
                    # if "rmse" in info:
                    #     MODEL_RMSE_GAUGE.labels(category=category, granularity=granularity, model_name=model_name).set(info["rmse"])
        else:
            MODEL_TRAINING_FAILURES.inc()
...
class RetrainRequest(BaseModel):
    file_path: str = "app/ITSM_data.csv" # Path to the data file for retraining

class DriftDetectionRequest(BaseModel):
    category: str
    granularity: str
    new_data: list # List of dictionaries, e.g., [{"ds": "2023-01-01", "y": 10}, ...]
    threshold_factor: float = 0.1

@app.get("/")
async def read_root():
    return {"message": "ITSM Incident Forecasting API is running. Visit /docs for API documentation."}
...
async def get_forecast(request: ForecastRequest):
    ...
    try:
        ...
        model_key = f"{request.category}_{request.granularity}"
        if model_key not in trained_models_info:
            raise HTTPException(status_code=404, detail=f"No model found for category '{request.category}' with granularity '{request.granularity}'.")

        model_info = trained_models_info[model_key]
        model_name = model_info["model_name"]
        
        # Load the model
        model = load_model(request.category, request.granularity, model_name)
        
        if model_name != 'Naive' and model is None:
            raise HTTPException(status_code=500, detail=f"Failed to load model '{model_name}' for {model_key}.")

        # Reconstruct full_df from data_snapshot for forecasting
        data_snapshot_json = model_info["data_snapshot"]
        full_df = pd.read_json(data_snapshot_json, orient='records', convert_dates=['ds'])
        
        # Ensure 'ds' is datetime and 'y' is numeric
        full_df['ds'] = pd.to_datetime(full_df['ds'])
        full_df['y'] = pd.to_numeric(full_df['y'])

        forecast_series = forecast_with_model(model, model_name, full_df, request.future_periods, request.granularity)
        
        # Convert forecast to a list of dictionaries for JSON response
        forecast_data = [
            {"date": str(date.date()), "predicted_value": round(value, 2)}
            for date, value in forecast_series.items()
        ]
        
        end_time = time.time()
        REQUEST_LATENCY.labels(method='POST', endpoint='/forecast').observe(end_time - start_time)
        return {"category": request.category, "granularity": request.granularity, "forecast": forecast_data}
        ...
        raise HTTPException(status_code=500, detail=f"An internal server error occurred during forecasting: {e}")

@app.post("/retrain")
async def retrain_models(background_tasks: BackgroundTasks, request: RetrainRequest):
    start_time = time.time()
    REQUEST_COUNT.labels(method='POST', endpoint='/retrain').inc()
    
    # Schedule retraining as a background task
    background_tasks.add_task(run_full_training_pipeline, file_path=request.file_path)
    
    end_time = time.time()
    REQUEST_LATENCY.labels(method='POST', endpoint='/retrain').observe(end_time - start_time)
    return {"message": "Model retraining initiated in the background. Check logs for progress."}

@app.post("/detect_drift")
async def detect_drift(request: DriftDetectionRequest):
    start_time = time.time()
    REQUEST_COUNT.labels(method='POST', endpoint='/detect_drift').inc()
    ...
    try:
        ...
        model_key = f"{request.category}_{request.granularity}"
        if model_key not in trained_models_info:
            raise HTTPException(status_code=404, detail=f"No historical data snapshot found for category '{request.category}' with granularity '{request.granularity}'.")

        historical_data_json = trained_models_info[model_key]["data_snapshot"]
        historical_df = pd.read_json(historical_data_json, orient='records', convert_dates=['ds'])
        historical_df['ds'] = pd.to_datetime(historical_df['ds']) # Ensure 'ds' is datetime
        historical_df['y'] = pd.to_numeric(historical_df['y']) # Ensure 'y' is numeric

        new_data_df = pd.DataFrame(request.new_data)
        new_data_df['ds'] = pd.to_datetime(new_data_df['ds']) # Ensure 'ds' is datetime
        new_data_df['y'] = pd.to_numeric(new_data_df['y']) # Ensure 'y' is numeric

        drift_detected = detect_data_drift(new_data_df, historical_df, request.threshold_factor)

        if drift_detected:
            DATA_DRIFT_DETECTED.inc()
            # Optionally trigger retraining if drift is detected
            # background_tasks.add_task(run_full_training_pipeline, file_path="app/ITSM_data.csv")
            # logger.warning("Data drift detected, retraining initiated in background.")
            return {"drift_detected": True, "message": "Data drift detected. Consider retraining models."}
        else:
            return {"drift_detected": False, "message": "No significant data drift detected."}

    except Exception as e:
        logger.error(f"Error during drift detection: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"An internal server error occurred during drift detection: {e}")

@app.get("/metrics")
async def get_metrics():
    # This endpoint is for Prometheus to scrape.
    # The start_http_server already exposes metrics on port 8001.
    # This /metrics endpoint on the main app is just a placeholder or for debugging.
    # Prometheus will directly scrape http://<your_app_ip>:8001/metrics
    return {"message": "Prometheus metrics are available on port 8001 at /metrics"}


if __name__ == "__main__":
    # For local development, you can run: python app/main.py
    # In production, uvicorn will be started by gunicorn or directly.
